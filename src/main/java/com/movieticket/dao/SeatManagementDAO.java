...
public class SeatManagementDAO {
	public boolean addSeat(SeatBean seat) {
		...
		try (Connection c = DBConnection.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
			p.setString(1, seat.getSeatId());
			p.setString(2, seat.getMallId());
			p.setString(3, seat.getSeatTypeId());
			p.setString(4, seat.getRowName());
			p.setInt(5, seat.getSeatNumber());
			p.setBoolean(6, seat.isActive());
	...
	public boolean updateSeat(SeatBean seat) {
		...
		try (Connection c = DBConnection.getConnection(); PreparedStatement p = c.prepareStatement(sql)) {
			p.setString(1, seat.getMallId());
			p.setString(2, seat.getSeatTypeId());
			p.setString(3, seat.getRowName());
			p.setInt(4, seat.getSeatNumber());
			p.setBoolean(5, seat.isActive());
			p.setString(6, seat.getSeatId());
			return p.executeUpdate() > 0;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public List<SeatBean> getSeatTypes() {
		...
		try (Connection c = DBConnection.getConnection();
			...
			while (r.next()) {
				...
				s.setSeatTypeId(r.getString("id"));
				s.setTypeName(r.getString("type_name"));
				list.add(s);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private SeatBean map(ResultSet r) throws Exception {
		SeatBean s = new SeatBean();
		s.setSeatId(r.getString("id"));
		s.setMallId(r.getString("mall_id"));
		s.setSeatTypeId(r.getString("seat_type_id"));
		s.setRowName(r.getString("row_name"));
		s.setSeatNumber(r.getInt("seat_number"));
		s.setActive(r.getBoolean("status"));
		s.setMallName(r.getString("mall_name"));
		s.setTypeName(r.getString("type_name"));
		return s;
	}
}